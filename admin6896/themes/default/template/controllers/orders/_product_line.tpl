...
<tr{if isset($product.image) && $product.image->id && isset($product.image_size)} height="{$product['image_size'][1] + 7}"{/if}>
	...
	<!--<td><span class="productName">{$smarty.get.code}</span></td>-->
	<td aling="center">
		<a href="pdfs.php?productoid={$product['product_id']}&productoinorder={$product['id_order_detail']}&orderid={($order->getOrderId())}"><input type="button" value = "Ver PDF - códigos"></a>
		
	</td>
	
	<td align="center">{if isset($product.image) && $product.image->id}{$product.image_tag}{/if}</td>
	<td aling ><a href="index.php?controller=adminproducts&id_product={$product['product_id']}&updateproduct&token={getAdminToken tab='AdminProducts'}">
...
<!-- Agrego la opcion de asignar codigo -->
	{if ($order->hasBeenPaid() && !$order->hasBeenDelivered())}
		<a href="codigoAsignar.php?product= {$product['product_name']}&productid={$product['product_id']}&productinorder={$product['id_order_detail']}&orderid={($order->getOrderId())}&cant={$product['product_quantity']}&url={$smarty.server.HTTP_HOST|escape:"url"}{$smarty.server.REQUEST_URI|escape:"url"}" ><img src="../img/admin/edit_2.gif"  alt="{l s='Assign Code'}" /></a>
